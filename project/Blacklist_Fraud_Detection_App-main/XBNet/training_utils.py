...
def predict(model,X):
    ...
    predicted value(int)
    '''
    # Ensure model is in eval mode and avoid grad tracking
    model.eval()
    ...
    if model.name == "Classification":
        # Binary (labelled as 1) -> apply sigmoid and threshold at 0.5
        if model.labels == 1:
            probs = torch.sigmoid(y_pred).cpu().detach()
            # return class (0 or 1)
            preds = (probs >= 0.5).long().cpu().numpy()
            # return scalar if single sample
            return int(preds.flatten()[0])
        else:
            # Multi-class -> argmax
            arr = y_pred.cpu().detach().numpy()
            return int(np.argmax(arr, axis=1).flatten()[0])
    else:
        return float(y_pred.cpu().detach().numpy().flatten()[0])
...
def predict_proba(model,X):
    ...
    '''
    # Return calibrated probability (0-1) for the positive/fraudulent class
    model.eval()
    X = torch.from_numpy(X)
    if X.dim() == 1:
        X = X.unsqueeze(0)
    ...
    if model.name == "Classification":
        if model.labels == 1:
            # Binary classification -> sigmoid to get probability for positive class
            probs = torch.sigmoid(y_pred).cpu().detach().numpy()
            # handle shapes like (1,1) or (1,)
            return float(probs.flatten()[0])
        else:
            # Multi-class -> softmax and return probability of class 1 if present
            probs = torch.nn.functional.softmax(y_pred, dim=1).cpu().detach().numpy()
            if probs.ndim == 2 and probs.shape[1] > 1:
                return float(probs[0, 1])
            return float(probs.flatten().max())
    else:
        # Regression: return prediction as float (not a probability)
        return float(y_pred.cpu().detach().numpy().flatten()[0])